...
def generate_images(
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        for angle_y, angle_p in [(.4, angle_p), (0, angle_p), (-.4, angle_p)]:
            ...
            lm, _ =  mtcnn.detect(img_arr)
            im_tensor, lm_tensor = read_data(im, lm, lm3d_std)
            data = {
            'imgs': im_tensor,
            'lms': lm_tensor
            }
            ...
            model.test()
            coeff = model.pred_coeffs_dict
            coeff_concat = []
            for item in coeff.items():
                coeff_concat.append(item)
            coeff_concat = torch.stack(coeff_concat)
            z_hat = MLP(coeff_concat)
            L2_loss = loss(z_hat, z)
